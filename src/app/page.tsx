          ...
          <div className="grid md:grid-cols-3 gap-6">
            {FEATURES.map((f, i) => (
              <Reveal key={f.title} delay={i * 60}>
                <div className={`h-full ${f.bgClass} rounded-2xl shadow-[var(--shadow-sm)] border border-ink/[0.05] overflow-hidden flex`}>
                  <div className={`w-1.5 ${f.color}`} />
                  <div className="p-7">
                    <div className="w-32 h-24 mb-4 overflow-hidden flex items-start justify-start">
                      {f.icon.startsWith("http") ? (
                        <img src={f.icon} alt={f.title} className="w-32 h-32 object-cover object-top" />
                      ) : (
